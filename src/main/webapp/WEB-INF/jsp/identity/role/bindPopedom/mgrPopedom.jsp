		...
		success:function(data){
			$.each(data,function(){
				if(this.code.length!=12){
					d.add(this.code,this.pcode,this.name,this.code.length==4?"javascript:void(0)":"javascript:refurbishRight('"+this.code+"')",this.name);
				}
...
function refurbishRight(code){
	   $("#sonModules").prop("src","${ctx}/identity/popedom/getModulesByParent.jspx?parentCode="+code+"&roleId=${role.id}");
}
